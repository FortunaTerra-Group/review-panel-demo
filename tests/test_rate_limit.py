...
def test_accepted_rate_over_any_window_span(client, tenants, fixed_clock):
    ...
    fixed_clock.advance(w - 1)                       # last second of what a fixed window would call bucket N
    for _ in range(3):
        assert get(client, tenants["retail"]).status_code == 200
    fixed_clock.advance(2)                           # first second of bucket N+1
    assert get(client, tenants["retail"]).status_code == 429
    fixed_clock.advance(w)                           # a full window later the burst has aged out
    assert get(client, tenants["retail"]).status_code == 200
...
@pytest.mark.django_db
def test_admin_invalidation_takes_effect_without_waiting_for_ttl(client, tenants):
...
def test_public_path_is_not_counted_and_touches_no_db(client, fake_redis):
    ...
    assert fake_redis.keys("ratelimit:None*") == []
